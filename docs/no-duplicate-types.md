...
The adapter supports direct imported `text`, `varchar`, `uuid`, `char`, `integer`, `int`, `serial`, `smallint`, `real`, `float`, `doublePrecision`, and `boolean` columns, plus SQLite `blob` columns. It recognizes direct `pgTable`, `mysqlTable`, and `sqliteTable` declarations, as well as `schema.table(...)` when `schema` is initialized with `pgSchema(...)`. It accounts for `notNull`, primary keys, and supported default methods when determining nullability and insert optionality. A single supported `$type<T>()` override determines the inferred column type; SQLite `integer(..., { mode: "boolean" })` infers `boolean`. It also resolves direct `typeof table.$inferSelect` and `typeof table.$inferInsert` aliases. Custom types, generated or identity columns, unsupported dialect-specific modes, and unsupported builder chains are skipped rather than guessed.
